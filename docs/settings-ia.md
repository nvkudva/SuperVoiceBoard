...
- [ ] **Languages** — the enabled languages, and per language: layout, multilingual typing,
...
- [ ] **Typing** — merges *Preferences*, *Text correction* and *Gesture typing*:
...
- [ ] **Voice** — dictation, with *Voice models* promoted from a row to a status card at the top
      (installed / needed / downloading) since it is the gate on the whole feature.
- [ ] **Look & feel** — splits today's 20-row Appearance into **Style** (theme, key style, icons,
      borders, background) and **Size & spacing** (the six scales, split), each with a live
      keyboard docked at the top.
- [ ] **Toolbar & keys** — the toolbar editor below, plus the key-behaviour settings that today
      sit under Advanced (long-press delay, spacebar swipes, delete swipe).
- [ ] **Privacy & data** — incognito, personalized suggestions, backup and restore, and the
...
- [ ] "Preferences" → **Typing & feedback**; "Secondary layouts" → **Symbol & number layouts**
- [ ] Four × "Switch to main keyboard after…" → "After symbols and a space", "After the numpad",
      "After an emoji", "After pasting"
- [ ] "More auto-correction" → **Correct more words**; "Always use middle suggestion" → **Keep the
      best word in the middle**; "Next-word suggestions" → **Suggest the next word**
- [ ] "Floating preview" / "Dynamic floating preview" → **Show the word while you swipe** /
      **Update it as you swipe**; "Phrase gesture" → **Swipe through the space bar for two words**
- [ ] "Show TLD popup keys" → **Show .com on the period key**; "Select hint source" → **Which popup
      letter to show**; "Split distance" → **Gap between the halves**
- [ ] "Remove discourse fillers" → **Remove "you know" and "I mean"**; "Save log" → **Save a
      diagnostic log**; "Override Emoji version" → **Emoji set version**
- [ ] Delete *Dictate in place* and *Google Password Manager fill*, or wire them to something

## The three flows worth rebuilding

### Toolbar keys — today: 5 taps, a 36-row dialog and a long-press drag
- [ ] Replace the three near-identical reorder dialogs with **one screen showing the real strip**,
      rendered at keyboard width in the live theme, and a drawer of the off keys under it.
- [ ] Drag from drawer into strip to add, within the strip to reorder, out to remove.
...
- [ ] Dock a **live keyboard** at the top of the colour editor and let the user tap a part of it —
...
- [ ] Sliders stay, under Advanced, now labelled with their current value.
