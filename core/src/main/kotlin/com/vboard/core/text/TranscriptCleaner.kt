...
class TranscriptCleaner {
    ...
    fun clean(request: CleanupRequest): CleanupResult {
        ...
        // VB-QA-05: collapsing a stutter can leave exactly a command phrase
        // ("scratch that scratch that" -> "scratch that"). Rendering that as text
        // is not idempotent: cleaning the output fires the command on the next
        // pass, and on the VB-124 double-cleanup path that deletes an utterance
        // the user already committed. Re-detect here, gated on the collapse being
        // what changed the utterance — a stage-3 conversion ("scratch that
        // period") must never be able to turn dictated words into a command.
        if (repetitionsCollapsed > 0) {
            detectUtteranceCommand(tokens)?.let {
