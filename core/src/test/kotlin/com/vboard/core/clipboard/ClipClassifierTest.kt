...
class ClipClassifierTest {
    ...
    @ValueSource(
        strings = [
            ...
            "4111\u20091111\u20091111\u20091111", // thin space
            "4111\u20131111\u20131111\u20131111", // en dash
            "4111\u20141111\u20141111\u20141111", // em dash
            "4111\u22121111\u22121111\u22121111", // minus sign
        ],
    )
    fun `a card grouped by a non-ASCII separator is session-only`(text: String) {
        assertKept(ClipClass.SESSION_ONLY, classify(text))
    }
    ...
    @ValueSource(
        strings = [
            "4111\u200B111111111111", // zero-width space
            "4111\u200C111111111111", // zero-width non-joiner
            "4111\uFEFF111111111111", // BOM / zero-width no-break space
            "4111\u00AD111111111111", // soft hyphen
        ],
    )
    fun `a card carrying an invisible code point is still session-only`(text: String) {
        assertKept(ClipClass.SESSION_ONLY, classify(text))
    }

    @Test
    fun `a card written in Arabic-Indic digits is session-only`() {
        // Same Visa test number in U+0660..U+0669. Every digit rule here means
        // general category Nd, so the script the digits are written in must not
        // decide whether the number reaches disk.
        val arabicIndic = "4111111111111111".map { ('\u0660' + (it - '0')) }.joinToString("")
        assertKept(ClipClass.SESSION_ONLY, classify(arabicIndic))
    }

    @Test
    fun `a one-time code written in Arabic-Indic digits is session-only`() {
        val arabicIndic = "483920".map { ('\u0660' + (it - '0')) }.joinToString("")
        assertKept(ClipClass.SESSION_ONLY, classify(arabicIndic))
    }

    @Test
    fun `a one-time code carrying a zero-width space is session-only`() {
        assertKept(ClipClass.SESSION_ONLY, classify("483\u200B920"))
    }
