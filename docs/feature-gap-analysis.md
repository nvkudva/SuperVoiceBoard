...
|---|---|---|---|---|---|---|---|---|
| Glide / swipe typing that works on install | Yes, default | Yes, default | Code present, gated on a user-supplied proprietary lib; off by default | P0 | L | No | Roughly half of Android users swipe. Ours silently does nothing, and the settings screen (`GestureTypingScreen.kt`) advertises seven tuning options for a feature that never runs. Needs an open decoder (FlorisBoard/Openboard-style) or a licensed one. | Ship an open decoder (port FlorisBoard's glide decoder or an AOSP-compatible one) so swipe works on install. Until it lands, hide `GestureTypingScreen` when `sHaveGestureLib` is false so we stop advertising a dead feature. |
