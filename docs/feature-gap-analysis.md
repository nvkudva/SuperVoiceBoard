...
| Voice usable immediately after install | Instant online; offline pack ~50 MB | Instant | Mic is inert until 610 MB downloads (~2 GB with the refiner) | P0 | M | First-run experience is a mic key that does nothing. Needs either a small first-run model, or defaulting to the system recogniser until packs land, with the on-device pack as the upgrade. |
| A build a user can install | Play Store | Preinstalled | No published build; CI artifacts only (`README.md` Status) | P0 | M | There is currently no path from "wants this" to "has this". Includes store assets — the repo has no screenshot of the voice strip. |
| Correction quality from a neural/LLM model | On-device LLM autocorrect | One UI 8 "AI autocorrect" | 2015-era AOSP n-gram JNI decoder | P1 | L | Typing accuracy is the first thing anyone judges a keyboard on, and both competitors have re-based theirs on a model in the last two years. We have a 0.5B model in-process already; the question is whether it can be brought to bear on typing, not just dictation. |
| Inline translation while typing | Yes, 120+ languages | Yes, "translation input" | None | P1 | M | Daily tool for bilingual users, and the one competitor feature with a clean on-device story (ML Kit offline pairs, ~30 MB each) that fits our no-network-in-the-IME constraint rather than fighting it. |
| Tone / style rewriting | Partial (Magic Compose, outside the keyboard) | Yes, Writing Assist: tone, grammar, phrasing | AI fix corrects but does not restyle | P1 | S | The LLM, the process split, the toolbar key, the attribution UI and the undo bookkeeping all exist (`AiFixController.kt`). Adding "make this formal/casual/shorter" is prompt and menu work, and it is the cheapest way to match Samsung's headline AI feature. |
| Field diagnostics for crashes | Google-scale telemetry | Samsung telemetry | None; `VoiceMetrics` is memory-only and off by default (R24) | P1 | S | We ship a keyboard with three processes, 3.8k lines of untested concurrency (`REVIEW.md`) and no way to learn that it died on a user's phone. A crash stack is not user content, so this does not breach §3.4. |
| Stylus handwriting → text | Yes, plus editing gestures (strike to delete, circle to select) | Yes, S Pen handwriting mode | None | P2 | L | Table stakes on tablets and any S Pen device; irrelevant on a mid-range phone. Segment gap, not a mainstream one. |
| GIF search | Yes | Yes | None | P2 | L | Heavily used in messaging, but every major messaging app has its own picker, and a Tenor integration means network, a third-party ToS and a content pipeline in `:ui` — expensive and off-strategy for a privacy-first keyboard. |
| Sentence-level prediction (Smart Compose) | Yes, LLM-backed | Predictive text | Bigram next-word from dictionaries only | P2 | M | Visible quality difference in long-form typing. Shares a foundation with the correction-quality row above; do them together or not at all. |
| Spoken punctuation, formatting and emoji commands | Yes ("comma", "new line", "smile emoji") | Partial | Spoken commands and auto-punctuation exist; coverage against Gboard's vocabulary is unverified | P2 | S | Dictation that cannot produce a newline or a question mark on command reads as a demo. Cheap to close because the command layer is already in `:core`. |
| Automatic language detection while typing | Yes, across enabled languages | Yes | Requires configuring a secondary locale per subtype (`SubtypeScreen.kt:155`) | P2 | M | We have the capability and hide it behind configuration most users will never find. Half a fix is a discoverability fix. |
| High-contrast / accessibility presets | Partial, via themes | Yes, explicit "High contrast keyboard" | Achievable by hand-editing custom colours; no preset | P2 | S | One preset in the appearance screen; the colour system already supports it. Low cost, real users. |
| Stickers and sticker suggestions | Yes | Yes | None | P3 | L | Same network/content-pipeline cost as GIF, less usage. |
| Emoji Kitchen / AR emoji | Emoji Kitchen | AR Emoji stickers | None | P3 | L | Fun, proprietary, and not why anyone switches keyboards. |
| Cross-device clipboard sync | Yes, via account | Yes, Samsung Cloud / Link to Windows | None — see §3 | P3 | L | Requires an account and a server. Structurally excluded by our own constraints. |
| In-keyboard web search | Yes | Regional | None | P3 | M | Requires network in the keyboard and is the feature most users disable. |
| Text scanning / OCR | No | Yes, "Scan text" | None | P3 | M | Samsung-only, camera permission, narrow use. |
| Settings and personal-dictionary sync | Yes, via account | Yes | Manual backup/restore file (`BackupRestorePreference.kt`) | P3 | L | Account-gated; our file-based backup covers the migration case, which is most of the value. |
| Physical / Bluetooth keyboard support | Full | Full | Partial (`PREF_ENABLE_EMOJI_ALT_PHYSICAL_KEY` only) | P3 | M | Tablet and desktop-mode segment. |
| Non-English UI strings still branded HeliBoard | n/a | n/a | ~100 `values-*/strings.xml` untouched (R2) | P3 | S | Every non-English user sees a different product's name in settings. Deliberate at the time, but it is now a trust problem, not a merge-hygiene one. |
