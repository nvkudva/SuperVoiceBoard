...
class QaRegressionPinTest {
    ...
    @Test
    fun `VB-QA-05 idempotency holds, including the last documented breaker`() {
        // Two of the three original breakers were closed earlier: the "no wait"
        // marker now requires i > 0, so the stacked form stops eating its own
        // leftover, and adjacent breaks are merged before rendering, so "\n\n\n"
        // is never emitted.
        for (input in listOf(
        ...
        // The last one is closed by re-detecting the utterance command after a
        // repetition collapse: the collapse leaves the bare phrase, so the first
        // pass returns the command itself rather than the text "Scratch that".
        // Nothing survives for a second pass to re-trigger, which is what the
        // VB-124 double-cleanup fallback path needs.
        val breaker = "scratch that scratch that"
        val first = clean(breaker)
        assertEquals(UtteranceCommand.SCRATCH_THAT, first.command)
        assertEquals("", first.text)
        assertEquals("", clean(first.text).text)
