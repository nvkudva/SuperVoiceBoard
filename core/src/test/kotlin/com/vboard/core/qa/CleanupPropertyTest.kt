...
class CleanupPropertyTest {
    ...
    fun `clean is idempotent over stacked-marker and break inputs`() {
        // (a) and (c) were closed earlier: the "no wait" marker requires i > 0
        //     (VB-QA-20), so the stacked form no longer eats its own leftover, and
        //     adjacent breaks are merged before rendering (VB-QA-30), so "\n\n\n"
        //     is never emitted. (b) is closed by re-detecting the utterance command
        //     after a repetition collapse: "scratch that scratch that" is now the
        //     SCRATCH_THAT command rather than the text "Scratch that", so there is
        //     nothing left for a second pass to re-trigger.
        for (input in listOf(
