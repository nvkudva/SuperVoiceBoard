...
class AiFixController(
    ...
    private var surface: FixSurface? = null
    ...
    /** Binds the key this controller drives. */
    fun attach(surface: FixSurface) {
    ...
    fun refresh() {
        surface?.let { it.updateFixButton(currentState(), describe(currentState())) }
    }
    ...
    /** The key's spoken description for [state]; the glyph is the surface's job. */
    private fun describe(state: FixButtonState): String = context.getString(
        when (state) {
    ...
    private fun say(resId: Int) {
        surface?.showFixMessage(context.getString(resId))
    }
