...
- [x] BUILD `app/build.gradle.kts:90` — `debug` runs R8 plus a per-variant proguard rewrite, and five build types each fan out to 5 ABI outputs → `runTests` and `nouserlib` are gone (CI runs the debugNoMinify tests); `debug` still minifies, which is HeliBoard's choice for APK size (S)
