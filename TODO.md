...
- [ ] Scope continuous AI fix to the text that changed, not the whole field. `AiFixController.scheduleAutoFix` fires at every word boundary once a fix has landed, and `performFix` reads and rewrites the entire field each time — so a long message costs one full-field LLM pass per word typed, and every pass can move the cursor. Options to weigh: correct only the completed word or the current sentence; reuse the sentence buffer that already tracks it; or keep the whole-field pass but run it on a longer idle and diff-apply so unchanged spans are never rewritten. Measure the per-pass cost on a real message before choosing.
