...
object KeyCode {
    ...
    const val TOGGLE_RESIZE_MODE =          -115 // WaveKey: interactive keyboard resize
    ...
    // WaveKey: the AI fix key (W5.1), in the app-exclusive range.
    const val AI_FIX =                       -10090
    // Long-press on the AI fix key: what the model changed (W5.2).
    const val AI_FIX_ATTRIBUTION =           -10091
    // WaveKey: flip dictation between the system recognizer and Parakeet.
    ...
    fun Int.checkAndConvertCode(longPress: Boolean = false): Int = if (this > 0) this else when (this) {
        ...
        TOGGLE_RESIZE_MODE, TOGGLE_ASR_ENGINE // WaveKey
        -> this
